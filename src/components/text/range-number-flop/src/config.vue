<template>
  <div class="setting-panel-gui">
    ...
    <g-field-collapse label="范围设置">
      <g-field :level="2" label="最小值">
        <g-input-number v-model="config.range.min" :min="0" :max="1000000" :step="1" />
      </g-field>
      <g-field :level="2" label="最大值">
        <g-input-number v-model="config.range.max" :min="0" :max="1000000" :step="1" />
      </g-field>
      <g-field :level="2" label="时间间隔">
        <g-input-number v-model="config.range.interval" :min="100" :max="60000" :step="100" suffix="ms" />
      </g-field>
    </g-field-collapse>
  </div>
  <g-field label="鼠标指针">
...
import { RangeNumberFlop } from './range-number-flop'

export default defineComponent({
  name: 'VRangeNumberFlopProp',
  props: {
    com: {
      type: Object as PropType<RangeNumberFlop>,
      required: true,
    },
  },
  setup(props) {
