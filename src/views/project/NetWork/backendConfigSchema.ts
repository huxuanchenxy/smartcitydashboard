...
export const BACKEND_TABLES: TableDef[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { prop: 'stepCode', label: '步骤编码', kind: 'text', width: 120 },
      { prop: 'callOrder', label: '调用顺序', kind: 'number', numberType: 'int', width: 100 },
      { prop: 'isForceSkip', label: '强制跳过', kind: 'boolean', width: 100 },
      { prop: 'isEnabled', label: '是否启用', kind: 'boolean', width: 100 },
      { prop: 'repeatPolicy', label: '重复策略', kind: 'json', inTable: false },
      { prop: 'preCheckSlots', label: '前置槽位校验', kind: 'json', inTable: false },
      { prop: 'inputMapping', label: '输入映射', kind: 'json', inTable: false },
      createdAtField,
      updatedAtField,
    ],
  },
